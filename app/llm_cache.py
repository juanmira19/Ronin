...
def _leer(k: str):
    ...
    if not f.exists():
        return None
    return json.loads(f.read_text(encoding="utf-8"))["interpretacion"]


def _escribir(k: str, payload: dict, interp: dict):
    CACHE_DIR.mkdir(parents=True, exist_ok=True)
    (CACHE_DIR / f"{k}.json").write_text(
        json.dumps({"clave": k,
                    "nota": payload["REPORTE_DEL_JUGADOR"]["nota"],
...
def interpretar(payload: dict, modo: str = "auto") -> dict:
    ...
    Agrega la clave interna `_fuente` (`modelo` | `cache`), que `run_prototype`
    saca del dict antes de usarlo para que no contamine el contrato."""
    k = clave(payload)
    ...
    if modo == "cache":
        ...
        if guardado is None:
            raise InterpretacionNoDisponible(
                "No hay interpretacion en cache para esta combinacion de RPE y nota. "
                "Corre `python -m scripts.precalentar_cache` con red para generarla.")
        return {**guardado, "_fuente": "cache"}

    try:
        interp = _interpretar_con_modelo(payload)
        _escribir(k, payload, interp)
        return {**interp, "_fuente": "modelo"}
        ...
        if guardado is None:
            raise InterpretacionNoDisponible(
                f"El modelo no respondio ({type(exc).__name__}) y no hay cache para "
                "esta combinacion de RPE y nota.") from exc
        return {**guardado, "_fuente": "cache"}
